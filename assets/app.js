const state = {
  ...
  adminFilters: {
    ...
    settingsAccountNickname: "",
    settingsAccountEmail: "",
    settingsAccountStatus: "全部",
    settingsAccountRole: "全部",
  },
  officialTaskDraft: {
...
const taskDetailProfile = {
  ...
  platformRequirementMap: {
    tiktok: {
      audienceGender: "女性消费人群优先",
      audienceAge: "18-30 岁",
      language: "英语 / 马来语口播优先",
      contentFormat: "短视频（30-60 秒）",
      duration: "30-60 秒，竖版 9:16",
      hook: "前 5 秒完成品牌露出与核心卖点口播",
      talkingPoints: "突出礼品属性、节日氛围与开箱体验",
      cta: "引导进入主页或评论区查看商品链接",
      scriptDeadline: "报名通过后 2 天内提交脚本",
      draftDeadline: "脚本通过后 3 天内提交初稿",
      publishWindow: "审核通过后 2 天内发布",
      linkBack: "发布后 24 小时内回传链接与首日数据截图",
      fileRule: "需提供字幕版视频与封面图",
      quote: "$120 - $180",
      settlementMethod: "固定价",
      settlementPeriod: "发布后 7 天回传数据结算",
      authorization: "默认含自然流量展示，不含额外广告投流授权",
    },
    youtube: {
      audienceGender: "家庭消费 / 泛生活方式受众优先",
      audienceAge: "20-35 岁",
      language: "英语讲解优先",
      contentFormat: "中长视频 / 开箱测评",
      duration: "6-10 分钟，横版 16:9",
      hook: "前 30 秒完成品牌与礼品场景引入",
      talkingPoints: "需完整介绍产品卖点、送礼场景与使用感受",
      cta: "描述栏挂链，并口播引导查看购买信息",
      scriptDeadline: "报名通过后 3 天内提交脚本大纲",
      draftDeadline: "脚本通过后 5 天内提交可审版本",
      publishWindow: "审核通过后 3 天内发布",
      linkBack: "发布后 48 小时内回传链接、封面与数据截图",
      fileRule: "需提供视频成片、封面与描述栏文案",
      quote: "$300 - $500",
      settlementMethod: "固定价",
      settlementPeriod: "发布后 14 天回传数据结算",
      authorization: "默认含站内自然展示，可单独追加广告白名单授权",
    },
  },
};
...
function opportunityCover(item) {
  ...
  const primaryPlatform = String(item.platform || "TikTok").split("/")[0].trim();
  const primaryRegion = String(item.region || "全球").split("/")[0].trim();
  return `
    <div class="cover cover-placeholder generated-opportunity-cover ${item.isOfficial ? "official-cover-placeholder" : ""}" aria-label="${item.name} 默认封面">
      <div class="cover-placeholder-copy">
        <strong>${item.name}</strong>
        <small>${primaryPlatform} · ${primaryRegion}</small>
      </div>
    </div>
  `;
}
...
const creatorCampaigns = [
  ...
  },
  {
    ...
    platform: "TikTok",
    deadline: "2026/06/30",
    cover: assets.opportunityRetail,
  },
  {
    name: "全境封锁2-上线活动",
    tag: "直播",
...
const settingsAccountRows = [
  { id: "admin-account-1", nickname: "运营-当前账号", email: "ops@meetinfluencer.com", status: "正常", statusClass: "running", role: "超级管理员", creator: "系统初始化", createdAt: "2026-05-18 09:20" },
  { id: "admin-account-2", nickname: "Amy", email: "amy@meetinfluencer.com", status: "正常", statusClass: "running", role: "审核员", creator: "运营-当前账号", createdAt: "2026-06-03 14:08" },
  { id: "admin-account-3", nickname: "Leon", email: "leon@meetinfluencer.com", status: "正常", statusClass: "running", role: "运营", creator: "运营-当前账号", createdAt: "2026-06-05 11:36" },
  { id: "admin-account-4", nickname: "Mika Finance", email: "finance@meetinfluencer.com", status: "停用", statusClass: "rejected", role: "财务", creator: "运营-当前账号", createdAt: "2026-05-28 16:22" },
];
...
const cooperationTypeOptions = ["全片", "插片", "直播", "长视频", "Shorts", "社区post", "视频下方link", "评论置顶", "视频投流", "素材授权", "官方代运营/社区运营"];

const cooperationTypeUnitMap = {
  全片: "分",
  插片: "秒",
  直播: "小时",
  长视频: "分",
...
function cooperationTypeUnit(type = "") {
  return cooperationTypeUnitMap[type] || "";
}

function cooperationTypeDisplay(type = "", amount = "") {
  if (!type) return "";
  const unit = cooperationTypeUnit(type);
...
const invitationSeedRows = [
  {
    ...
    advertiser: "平台官方",
    creator: "Mika Studio",
    inviteSentAt: new Date(Date.now() - 26 * 60 * 60 * 1000).toISOString(),
    inviteExpiresAt: new Date(Date.now() + 22 * 60 * 60 * 1000).toISOString(),
    type: "短视频",
    settlement: "固定价",
    quote: "99",
    quoteType: "fixed",
  ...
  },
  {
    ...
    advertiser: "品牌商家",
    creator: "Luna Trend",
    inviteSentAt: new Date(Date.now() - 50 * 60 * 60 * 1000).toISOString(),
    inviteExpiresAt: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
    type: "短视频",
    settlement: "固定价",
    quote: "120 - 180",
    quoteType: "range",
  ...
  },
  {
    ...
    advertiser: "平台官方",
    creator: "Nova Plays",
    inviteSentAt: new Date(Date.now() - 72 * 60 * 60 * 1000).toISOString(),
    type: "直播",
    settlement: "固定价",
    quote: "80",
    quoteType: "fixed",
...
function replacePersistedCollection(target, source) {
...
function hydrateRuntimeState() {
  try {
    ...
    Object.entries(persistedCollections).forEach(([key, target]) => {
...
function persistRuntimeState() {
  try {
    const payload = {
      state: Object.fromEntries(persistedStateKeys.map((key) => [key, state[key]])),
      collections: Object.fromEntries(Object.entries(persistedCollections).map(([key, value]) => [key, value])),
    };
    window.localStorage.setItem(runtimeStorageKey, JSON.stringify(payload));
  } catch (error) {
    console.warn("persist runtime state failed", error);
  }
}
...
function render() {
  syncCampaignStatusLabels();
  syncOpportunityCoverSources();
  syncInvitationTimeoutStates();
  renderShellState();
  renderTopbar();
  const renderer = pages[state.page] || pages.promotions;
  pageRoot.innerHTML = renderer();
  syncInvitationCountdownTimer();
  persistRuntimeState();
}

function syncInvitationCountdownTimer() {
  const hasPendingInvites = invitations.some((row) => row.status === "待接单");
  if (state.page !== "invitations" || !hasPendingInvites) {
    if (invitationCountdownTimer) {
      window.clearInterval(invitationCountdownTimer);
      invitationCountdownTimer = null;
    }
    return;
  }
  if (invitationCountdownTimer) return;
  invitationCountdownTimer = window.setInterval(() => {
    syncInvitationTimeoutStates();
    render();
  }, 60000);
}
...
function displayTalentPriceRange(row = {}) {
  if (row.priceRange) return row.priceRange;
  const value = parseMoneyUSD(row.price);
  if (!value) return "-";
  const lower = Math.max(0, Math.round((value * 0.8) / 10) * 10);
  const upper = Math.round((value * 1.2) / 10) * 10;
...
function formatInvitationCountdown(ms = 0) {
...
function syncCampaignStatusLabels() {
  campaigns.forEach((row) => {
    if (row.status === "审核中") row.status = "待审核";
    if (row.status === "已到期") row.status = "已结束";
  });
  if (state.promotionFilters.status === "审核中") state.promotionFilters.status = "待审核";
  if (state.promotionFilters.status === "已到期") state.promotionFilters.status = "已结束";
}

function syncOpportunityCoverSources() {
  const coverMap = {
    "MINISO 夏季新品内容合作": assets.opportunityRetail,
    "全境封锁2-上线活动": assets.opportunityLive,
    "椰子水夏季饮品种草": assets.opportunityRetail,
  };
  creatorCampaigns.forEach((row) => {
    if (!row.cover && coverMap[row.name]) {
      row.cover = coverMap[row.name];
    }
  });
}

function syncInvitationTimeoutStates() {
  const now = Date.now();
  invitations.forEach((row) => {
    if (row.inviteSentAt) row.time = formatDateTimeLabel(row.inviteSentAt);
    if (row.status === "已合作" || row.status === "已拒绝" || row.status === "已失效") {
      if (row.status === "已失效") {
        row.statusClass = "expired";
        row.timeoutTip = "达人48小时未接受，邀请自动失效，已释放名额。";
      }
      return;
    }
    if (!row.inviteSentAt) return;
    if (!row.inviteExpiresAt) {
      row.inviteExpiresAt = new Date(new Date(row.inviteSentAt).getTime() + 48 * 60 * 60 * 1000).toISOString();
    }
    const expireAt = new Date(row.inviteExpiresAt).getTime();
    ...
    if (row.status === "待接单") {
      ...
      row.timeoutTip = `剩余 ${formatInvitationCountdown(expireAt - now)}`;
    }
  });
}

function invitationDeadlineLabel(row = {}) {
  if (row.inviteExpiresAt) return formatDateTimeLabel(row.inviteExpiresAt);
  if (row.inviteSentAt) {
    return formatDateTimeLabel(new Date(new Date(row.inviteSentAt).getTime() + 48 * 60 * 60 * 1000).toISOString());
  }
  return "-";
}

function pushInboxNotification(bucket, item) {
...
function notifyBrand(item) {
  pushInboxNotification(state.brandNotifications, item);
}

function notifyCreator(item) {
...
function firstCountryCode(value = "") {
  return String(value || "")
    .split("/")
    .map((item) => item.trim())
    .filter(Boolean)[0] || "US";
...
function findAdminCampaignRecord(taskName = "") {
  return adminAdvertiserCampaignRows.find((item) => item.name === taskName);
}

function findCreatorCampaignRecord(taskName = "") {
  return creatorCampaigns.find((item) => item.name === taskName);
}
...
function findProjectRecord(taskName = "") {
  return projectRows.find((item) => item.task === taskName);
}

function findCollaborationRecord(taskName = "", creatorName = "") {
  return collaborations.find((item) => item.campaign === taskName && (!creatorName || item.creator === creatorName));
}

function findFulfillmentRecord(taskName = "", creatorName = "") {
  return adminCampaignFulfillmentRows.find((item) => item.campaign === taskName && (!creatorName || item.creator === creatorName));
}

function buildTaskProfile(taskName = "") {
  const campaignRow = findCampaignRecord(taskName) || {};
  const adminRow = findAdminCampaignRecord(taskName) || {};
  const creatorRow = findCreatorCampaignRecord(taskName) || {};
  return {
    taskName,
    advertiser: adminRow.advertiser || "当前广告主",
    platform: creatorRow.platform || adminRow.platform || "TikTok",
    region: creatorRow.region || campaignRow.region || "US / UK",
    budget: creatorRow.budget || adminRow.budget || campaignRow.budget || "$0",
    requirement: creatorRow.requirement || adminRow.requirement || "粉丝数≥20万",
    cooperationType: adminRow.cooperationType || "短视频",
    deadline: creatorRow.deadline || campaignRow.start || formatToday(),
  };
}

function platformDisplayName(value = "") {
  const text = String(value || "").trim();
  const key = invitePlatformKey(text);
  const map = { tiktok: "TikTok", youtube: "YouTube", instagram: "Instagram", twitch: "Twitch", facebook: "Facebook", twitter: "Twitter" };
  return map[key] || text;
}

function invitePlatformKey(value = "") {
  const text = String(value || "").trim().toLowerCase();
  if (text.includes("tiktok")) return "tiktok";
  if (text.includes("youtube")) return "youtube";
  if (text.includes("instagram") || text === "ins" || text.includes(" ins")) return "instagram";
...
function matchingInvitePlatforms(taskName = "", profiles = []) {
  const campaignPlatforms = platformTokens(buildTaskProfile(taskName).platform);
  const creatorPlatforms = [...new Set(profiles.map((profile) => platformDisplayName(profile.platform)).filter(Boolean))];
  if (!creatorPlatforms.length) return campaignPlatforms;
  return campaignPlatforms.filter((platform) => creatorPlatforms.some((creatorPlatform) => invitePlatformKey(creatorPlatform) === invitePlatformKey(platform)));
}

function inviteCampaignDisabledReason(taskName = "", profiles = []) {
  const campaignPlatforms = platformTokens(buildTaskProfile(taskName).platform);
  if (matchingInvitePlatforms(taskName, profiles).length) return "";
  return `当前活动仅限 ${campaignPlatforms.join(" / ") || "指定平台"} 推广，与达人平台不一致`;
}
...
function taskPlatformRequirementTemplate(platformName = "") {
  const key = normalizePlatformKey(platformName);
  const fallback = {
    audienceGender: "消费兴趣人群优先",
    audienceAge: "18-30 岁",
    language: "当地语言 / 英语内容优先",
    contentFormat: "平台原生内容",
    duration: "按平台内容规范执行",
    hook: "开头需完成品牌露出与核心卖点说明",
    talkingPoints: "围绕产品卖点、使用场景与转化引导展开",
    cta: "需提供清晰跳转与购买引导",
    scriptDeadline: "报名通过后 2 天内提交脚本",
    draftDeadline: "脚本通过后 3 天内提交初稿",
    publishWindow: "审核通过后按排期完成发布",
    linkBack: "发布后 24 小时内回传链接与数据截图",
    fileRule: "需回传成片、封面与数据截图",
    quote: "$100 - $200",
    settlementMethod: "固定价",
    settlementPeriod: "按合作协议约定结算",
    authorization: "默认含自然流量展示授权",
  };
  return taskDetailProfile.platformRequirementMap?.[key] || fallback;
}

function taskPlatformRequirement(profile, platformItem) {
  const platformName = platformItem?.name || "TikTok";
  const template = taskPlatformRequirementTemplate(platformName);
  const draftConfig = state.publishDraft.platformConfigs?.[platformName] || null;
  const followerRequirement =
    draftConfig?.followerMinK && draftConfig?.followerMaxK
      ? `粉丝数 ${draftConfig.followerMinK}-${draftConfig.followerMaxK} 万`
      : profile.followerRequirement || template.followerRequirement || "-";
  const creatorTypes = draftConfig?.creatorTypes?.length ? draftConfig.creatorTypes : profile.creatorTypes?.length ? profile.creatorTypes : [];
  const contentFormat =
    draftConfig?.cooperationTypes?.length
      ? cooperationTypeDisplay(draftConfig.cooperationTypes[0] || "", draftConfig.cooperationValue || "")
      : template.contentFormat || profile.contentType || "-";
  const quote =
    draftConfig?.unitPriceMin && draftConfig?.unitPriceMax
      ? `${formatUsdBudget(draftConfig.unitPriceMin)} - ${formatUsdBudget(draftConfig.unitPriceMax)}`
      : template.quote || profile.budget || "-";
  return {
    platformName,
    audienceGender: template.audienceGender,
    audienceAge: template.audienceAge,
    ...
    hook: template.hook,
    talkingPoints: draftConfig?.contentRequirement || template.talkingPoints,
    cta: template.cta,
    scriptDeadline: template.scriptDeadline,
    draftDeadline: template.draftDeadline,
    publishWindow: template.publishWindow,
    linkBack: template.linkBack,
    fileRule: template.fileRule,
    quote,
    settlementMethod: draftConfig?.settlement || template.settlementMethod || profile.settlementMethod || "-",
    settlementPeriod: draftConfig?.settlementCycle || template.settlementPeriod || profile.settlementPeriod || "-",
    authorization: template.authorization,
  };
}

function ensureCreatorCampaignVisible(taskName = "") {
  if (!taskName || findCreatorCampaignRecord(taskName)) return;
  const profile = buildTaskProfile(taskName);
  creatorCampaigns.unshift({
    name: taskName,
    tag: profile.cooperationType,
    tags: [profile.cooperationType, "达人合作", "平台审核通过"],
...
function ensureCampaignLifecycleRowFromDraft() {
  const taskName = state.publishDraft.campaignName.trim();
  if (!taskName) return "";
  ensurePublishDraftPlatforms();
  const platforms = state.publishDraft.platforms.length ? state.publishDraft.platforms.join(" / ") : "TikTok";
  const countries = state.publishDraft.countries.length ? state.publishDraft.countries : ["英国"];
  const region = countries.map((item) => countryCodeFromName(item) || item).join(" / ");
  const firstPlatform = state.publishDraft.platforms[0] || "TikTok";
  const config = state.publishDraft.platformConfigs[firstPlatform] || defaultPublishPlatformConfig(firstPlatform);
  const totalBudget =
    state.publishDraft.platforms.reduce((sum, platform) => sum + normalizeMoneyNumber(state.publishDraft.platformConfigs[platform]?.platformBudget), 0) ||
    normalizeMoneyNumber(config.platformBudget);
  const budgetText = `$${totalBudget.toLocaleString("en-US")}`;
  const campaignRow = findCampaignRecord(taskName);
  const baseCampaign = {
    name: taskName,
    product: state.publishDraft.productName || "推广产品",
    region: countries.join("、"),
    joined: campaignRow?.joined || 0,
    publishedCreators: campaignRow?.publishedCreators || 0,
    works: campaignRow?.works || 0,
    budget: budgetText,
    start: formatNow(),
    status: "审核中",
    statusClass: "pending",
  };
  if (campaignRow) Object.assign(campaignRow, baseCampaign);
  else campaigns.unshift(baseCampaign);
  ...
  const baseAdminRow = {
    ...
    name: taskName,
    platform: platforms,
    creators: adminRow?.creators || 0,
    published: adminRow?.published || 0,
    budget: budgetText,
    status: "待审核",
    statusClass: "pending",
    requirement: config.followerMinK ? `粉丝数≥${config.followerMinK}K` : "粉丝数≥20万",
    cooperationType: cooperationTypeDisplay((config.cooperationTypes || [])[0] || "", config.cooperationValue) || "短视频",
  };
  if (adminRow) Object.assign(adminRow, baseAdminRow);
  else adminAdvertiserCampaignRows.unshift(baseAdminRow);
  return taskName;
}

function appendProjectTimeline(taskName = "", title = "", detail = "", result = "") {
...
function appendFulfillmentTimeline(taskName = "", creatorId = "", title = "", detail = "") {
  if (!taskName || !creatorId) return;
  const key = `${taskName}::${creatorId}`;
  if (!adminFulfillmentTimelineMap[key]) adminFulfillmentTimelineMap[key] = [];
  adminFulfillmentTimelineMap[key].push({
    title,
    desc: `${formatNow()} ${detail}`,
  });
}

function ensureCooperationRecords(taskName = "", creatorName = "", options = {}) {
  if (!taskName || !creatorName) return;
  const profile = buildTaskProfile(taskName);
  const creator = findCreatorProfile(creatorName);
  const existingCollaboration = findCollaborationRecord(taskName, creatorName);
  const creatorId = options.creatorId || findFulfillmentRecord(taskName, creatorName)?.creatorId || `${creatorCode(creatorName)}-${Date.now().toString().slice(-4)}`;
  const quoteNumber = String(options.quote || creator.price || "$0").replace(/[^\d.]/g, "") || "0";
  const cooperationType = options.type || profile.cooperationType || "短视频";
  const collaboration =
    existingCollaboration ||
    (collaborations.unshift({
      creator: creatorName,
      creatorId,
      avatar: creator.avatar || assets.creatorPreview,
      platform: creator.platform || profile.platform,
      fans: creator.fans || "-",
      contentTags: [creator.category || "内容达人"],
      audienceRegion: firstCountryRegion(creator.country),
      audienceGender: creator.gender || "-",
      audienceAge: creator.age || "-",
      recentVideo: "待提交交付内容",
      avgViews: creator.avgViews || "-",
      campaign: taskName,
      campaignInfo: {
        name: taskName,
        product: findCampaignRecord(taskName)?.product || "推广产品",
        platform: profile.platform,
        region: profile.region,
        budget: profile.budget,
        deadline: profile.deadline,
      ...
      statusClass: "running",
      progress: "待提交产物",
      due: profile.deadline,
      acceptTime: formatNow(),
      note: "已确认合作，等待达人提交内容。",
      deliverables: [],
    }) &&
      collaborations[0]);
  Object.assign(collaboration, {
    creator: creatorName,
    creatorId,
    avatar: creator.avatar || assets.creatorPreview,
    platform: creator.platform || profile.platform,
    ...
    progress: "待提交产物",
    due: profile.deadline,
    acceptTime: collaboration.acceptTime || formatNow(),
    note: "已确认合作，等待达人提交内容。",
  });
  ...
  const campaignRow = findCampaignRecord(taskName);
  if (campaignRow) {
    campaignRow.status = "进行中";
    campaignRow.statusClass = "running";
    if (!existingCollaboration) campaignRow.joined = Number(campaignRow.joined || 0) + 1;
  }
  const adminCampaign = findAdminCampaignRecord(taskName);
  if (adminCampaign) {
    adminCampaign.status = "进行中";
    adminCampaign.statusClass = "running";
    if (!existingCollaboration) adminCampaign.creators = Number(adminCampaign.creators || 0) + 1;
  }

  appendFulfillmentTimeline(taskName, creatorId, "确认合作", "达人已确认合作，进入内容创作。");
}

function ensureSettlementRecord(taskName = "", creatorName = "", amount = "0") {
  const existing = payoutRows.find((item) => item.campaign === taskName && item.creator === creatorName);
  if (existing) {
    existing.amount = formatUsdAmount(amount);
    existing.status = "待打款";
    existing.statusClass = "pending";
    existing.note = "系统已生成结算单，等待运营线下打款。";
    return existing;
  }
  const nextId = `payout-${payoutRows.length + 1}`;
  const row = {
    id: nextId,
    campaign: taskName,
    subtitle: `${taskName} 结算单`,
    creator: creatorName,
    amount: formatUsdAmount(amount),
    account: `PAYPAL ${state.revenuePayoutDraft.email}`,
    status: "待打款",
    statusClass: "pending",
    note: "系统已生成结算单，等待运营线下打款。",
    rejectReason: "",
  };
  payoutRows.unshift(row);
  return row;
}

function creatorCampaignWorkflowState(taskName = "") {
  const project = findProjectRecord(taskName);
  if (project) {
  ...
  const statusView = applicationStatusView(application.statusKey);
  return {
    label: application.statusKey === "rejected" ? "报名失败" : statusView.label,
    cls: statusView.cls,
    statusKey: application.statusKey,
  };
}
...
function hasBrandWorkspaceCampaigns() {
  return state.brandHasCampaigns || adminAdvertiserCampaignRows.some((row) => row.advertiser === "当前广告主");
}

function getFilteredCampaigns() {
  const source = hasBrandWorkspaceCampaigns() ? campaigns : [];
  ...
  return source
    ...
    .sort((a, b) => {
      const aRejected = a.status === "已驳回" ? 0 : 1;
      const bRejected = b.status === "已驳回" ? 0 : 1;
...
function campaignRejectReason(taskName = "") {
  return (
    adminReviewHistoryRows.find((row) => row.reviewType === "任务审核" && row.taskName === taskName && row.result === "已拒绝")?.reason ||
    "当前活动已被驳回，请补充活动信息后重新提交。"
  );
}

function rejectedCampaignRows() {
  return campaigns.filter((row) => row.status === "已驳回");
}

function preloadPublishDraftForCampaign(taskName = "") {
  const campaign = findCampaignRecord(taskName);
  const adminRow = findAdminCampaignRecord(taskName);
  const creatorRow = findCreatorCampaignRecord(taskName);
  if (!campaign && !adminRow && !creatorRow) return;
  const regionNames = String(campaign?.region || "")
    .split(/[、,/]/)
    .map((item) => item.trim())
    .filter(Boolean)
    .map((item) => countryNameFromCode(item) || item);
  const platformNames = String(creatorRow?.platform || adminRow?.platform || "")
    .split("/")
    .map((item) => item.trim())
    .filter(Boolean);
  state.publishDraft.campaignName = taskName;
  state.publishDraft.productName = campaign?.product || state.publishDraft.productName;
  state.publishDraft.platforms = platformNames.length ? platformNames : state.publishDraft.platforms;
  state.publishDraft.countries = regionNames.length ? regionNames : state.publishDraft.countries;
  state.publishDraft.areas = [];
  ensurePublishDraftPlatforms();
}
            ...
            ${
              isRejected
                ...
                : `${isDraft ? `<button class="link-button" data-action="open-publish">编辑</button>` : ""}
                   ...
                   ${isDraft ? `<button class="link-button" data-action="toast" data-message="已将 ${row.name} 移入删除确认队列">删除</button>` : ""}`
            }
          </div>
        </td>
      </tr>
...
function renderRejectedCampaignQueue() {
  const rows = rejectedCampaignRows();
  if (!rows.length) return "";
  return `
    <div class="panel rejected-campaign-panel">
      <div class="table-head rejected-campaign-head">
        <div>
          <h2>待重新提交</h2>
          <p class="subcopy">以下活动被运营驳回，建议优先补充信息后重新提交。</p>
        </div>
        <span class="status rejected">共 ${rows.length} 个待处理</span>
      </div>
      <div class="rejected-campaign-list">
        ${rows
          .map(
            (row) => `
            <article class="rejected-campaign-card">
              <div class="rejected-campaign-main">
                <strong>${row.name}</strong>
                <span>${row.start}</span>
...
function talentRows(source = creators) {
  const q = state.query.trim().toLowerCase();
  const selectedCreators = new Set(state.talentInviteSelections || []);
  ...
  return source
    ...
    .map(
      ...
      <tr>
        ...
        <td>${row.recent}</td>
        <td>${row.avgViews}</td>
        <td>${displayTalentPriceRange(row)}</td>
        <td>
          <div class="actions talent-row-actions">
            ...
            <button class="link-button" data-action="open-talent-search-invite" data-creator="${encodeURIComponent(row.name)}">邀请</button>
          </div>
        </td>
      </tr>
    `,
  ...
  return visible
    ...
    .map(
      ...
      <tr>
        ...
        <td>${invitationDeadlineLabel(row)}</td>
        <td>
          <div class="progress-cell">
            ${status(row.status, row.statusClass)}
            ${row.timeoutTip ? `<small class="muted">${row.timeoutTip}</small>` : ""}
          </div>
        </td>
        <td>
          <div class="actions">
        ...
        <td>
          <div class="actions">
            ${
              row.status === "已完成"
                ? ""
                : `<button class="link-button" data-action="modal-upload" data-task="${encodeURIComponent(row.task)}">${row.progress === "待发布" ? "提交链接" : "提交产物"}</button>`
            }
...
function renderPromotions() {
  ...
  const showOnboardingEmpty = !hasBrandWorkspaceCampaigns();
  const showFilteredEmpty = !showOnboardingEmpty && filteredCampaigns.length === 0;
  const rejectedCount = rejectedCampaignRows().length;
  return `
    ${pageHeader(
      "广告主工作台",
      "活动管理",
      ...
      <div class="table-card">
        <div class="table-head">
          <div>
            ...
            ${showOnboardingEmpty ? "" : `<span class="muted">进行中 / 历史活动统一管理${rejectedCount ? `，其中 ${rejectedCount} 个待重新提交` : ""}</span>`}
          </div>
          ${
            showOnboardingEmpty
              ...
              : `
                <div class="filter-bar" style="margin:0">
                  ...
                  <select class="select" data-promotion-filter="status">
                    ...
                    <option value="已结束" ${state.promotionFilters.status === "已结束" ? "selected" : ""}>已结束</option>
                    <option value="待审核" ${state.promotionFilters.status === "待审核" ? "selected" : ""}>待审核</option>
                    <option value="已驳回" ${state.promotionFilters.status === "已驳回" ? "selected" : ""}>已驳回</option>
                  </select>
                  <input
                    class="input"
                    type="date"
            ...
            : `
              <div class="table-wrap sticky-cols">
                <table class="campaign-table">
                  <thead>
                    <tr>
                      <th>活动名称</th><th>推广地区</th><th>推广平台</th><th>合作达人</th><th>已发布达人数</th><th>作品数</th><th>预算</th><th>活动发起时间</th><th>状态</th><th>操作</th>
                    </tr>
                  </thead>
                  <tbody>${showFilteredEmpty ? emptyRow(10) : campaignRows(filteredCampaigns)}</tbody>
                </table>
              </div>
            `
        }
  ...
  return `
    ...
    <div class="publish-page-form">
      ...
      <div class="panel publish-page-actions">
        ...
        <button class="primary-button" data-action="submit-publish">确认提交</button>
      </div>
    </div>
      ...
      <div class="publish-platform-config-list">
        ${draft.platforms
          .map((platform, index) => {
            ...
            const selectedCooperationType = (config.cooperationTypes || [])[0] || "";
            const selectedCooperationUnit = cooperationTypeUnit(selectedCooperationType) || "单位";
            return `
              <article class="publish-platform-config-card">
                ...
                <div class="form-grid">
                  ...
                  ${field(
                    "合作类型",
                    `<div class="cooperation-type-inline">
                      <select class="select" data-platform-name="${platform}" data-platform-field="cooperationTypeSingle">
                        <option value="">请选择合作类型</option>
                        ${cooperationTypeOptions.map((item) => `<option value="${item}" ${selectedCooperationType === item ? "selected" : ""}>${item}</option>`).join("")}
                      </select>
                      <div class="money-single cooperation-value-input">
                        <input class="input" placeholder="请输入数值" inputmode="decimal" value="${config.cooperationValue || ""}" data-platform-name="${platform}" data-platform-field="cooperationValue" />
                        <span class="money-suffix">${selectedCooperationUnit}</span>
                      </div>
                    </div>`,
                    true,
                    "full",
...
function publishPlatformConfigStatus(config) {
  ...
  const doneCount = [
    ...
    String(config.cooperationValue || "").trim(),
...
function publishForm(inModal = true) {
  ...
  const activeCooperationType = (activeConfig.cooperationTypes || [])[0] || "";
  const activeCooperationUnit = cooperationTypeUnit(activeCooperationType) || "单位";
                ...
                ${field(
                  ...
                  `<div class="cooperation-type-inline">
                    <select class="select" data-platform-field="cooperationTypeSingle">
                      <option value="">请选择合作类型</option>
                      ${cooperationTypeOptions.map((item) => `<option value="${item}" ${activeCooperationType === item ? "selected" : ""}>${item}</option>`).join("")}
                    </select>
                    <div class="money-single cooperation-value-input">
                      <input class="input" placeholder="请输入数值" inputmode="decimal" value="${activeConfig.cooperationValue || ""}" data-platform-field="cooperationValue" />
                      <span class="money-suffix">${activeCooperationUnit}</span>
                    </div>
                  </div>`,
                  true,
                  "full",
  ...
  return `
    <div class="modal-foot modal-foot-split" ${inModal ? 'id="publishModalFoot"' : ""} style="${
      ...
      <div class="modal-foot-right">
        ...
        <button class="primary-button" data-action="${step === 2 ? "submit-publish" : "publish-next"}">${step === 2 ? "确认提交" : "下一步"}</button>
      </div>
    </div>
  `;
}
...
function renderTalentSearch() {
  ...
  return `
    ...
    <div class="table-card talent-list-card">
      <div class="table-head"><h2>达人列表</h2><button class="primary-button" data-action="open-talent-search-invite">批量邀请</button></div>
      <div class="table-wrap talent-list-scroll">
        <table>
          <thead><tr><th><input type="checkbox" aria-label="全选达人" data-action="toggle-talent-invite-select-all" ${allSelected ? "checked" : ""} /></th><th>达人信息</th><th>国家/地区</th><th>粉丝数</th><th>受众性别</th><th>受众年龄</th><th>最近发布视频</th><th>平均曝光量</th><th>报价($)</th><th>操作</th></tr></thead>
  ...
  return `
    ...
    <div class="cards-grid">
      ${filteredCampaigns
        .map(
          (item) => {
            const applyState = creatorCampaignWorkflowState(item.name);
            return `
        <article class="campaign-card opportunity-card opportunity-card-clickable" data-action="open-task-detail" data-campaign="${encodeURIComponent(item.name)}" data-status="${applyState.statusKey}">
          ...
          <div class="campaign-price-highlight">
            ...
            ${
              applyState.statusKey === "none"
                ? `<button class="primary-button campaign-apply-button" data-action="modal-apply" data-campaign="${encodeURIComponent(item.name)}">立即报名</button>`
                : applyState.statusKey === "rejected"
                  ? `<span class="secondary-button campaign-apply-button rejected" aria-disabled="true">${applyState.label}</span>`
                  : `<span class="status ${applyState.cls}">${applyState.label}</span>`
            }
          </div>
          <div class="mini-stats">
...
function renderTaskDetailBody(showApply = true) {
  ...
  const matchedPlatforms = String(matchedCampaign?.platform || "")
    .split("/")
    .map((item) => item.trim())
    .filter(Boolean)
    .map((item) => ({
      ...
      name: item,
      logo: item.toLowerCase().includes("youtube")
        ? assets.youtubeLogo
        : item.toLowerCase().includes("instagram")
          ? assets.instagramLogo
          : item.toLowerCase().includes("twitch")
            ? assets.twitchLogo
            : assets.tiktokLogo,
    }));
  const detailProfile = {
    ...
    deadline: matchedCampaign?.deadline || taskDetailProfile.deadline,
    followerRequirement: matchedCampaign?.requirement || taskDetailProfile.followerRequirement,
    budget: matchedCampaign?.budget || taskDetailProfile.budget,
    creatorTypes: matchedCampaign?.tags?.length ? matchedCampaign.tags.slice(0, 3) : taskDetailProfile.creatorTypes,
    targetCountries: matchedCampaign?.region ? regionTextToCountries(matchedCampaign.region) : taskDetailProfile.targetCountries,
    platforms: matchedPlatforms.length ? matchedPlatforms : taskDetailProfile.platforms,
    contentType: matchedCampaign?.tag || taskDetailProfile.contentType,
    settlementType: matchedCampaign?.tag || taskDetailProfile.settlementType,
  };
  const activePlatform =
    detailProfile.platforms.find((item) => item.key === state.taskDetailPlatform) ||
    detailProfile.platforms[0] ||
    { key: "tiktok", name: "TikTok", logo: assets.tiktokLogo };
  const platformRequirement = taskPlatformRequirement(detailProfile, activePlatform);
  return `
    <div class="task-detail-layout">
      ...
      <div class="panel task-section-card">
        <div class="task-platform-head">
          <div>
            <h2>按平台查看要求</h2>
            <p class="subcopy">不同平台的达人门槛分开展示，避免混淆。</p>
          </div>
          <span class="task-platform-active-label">当前查看：${activePlatform.name}</span>
        </div>
        ${
          detailProfile.platforms.length > 1
            ? `
              <div class="task-platform-tabs">
                ${detailProfile.platforms
                  .map(
                    (platform) => `
                      <button class="task-platform-tab ${platform.key === activePlatform.key ? "active" : ""}" data-action="set-task-detail-platform" data-platform="${platform.key}">
        ...
        <div class="task-platform-card-grid">
          <section class="task-platform-card task-platform-card-wide">
            <h3>达人要求</h3>
            <div class="task-platform-list task-platform-list-horizontal">
              <div class="task-platform-list-item"><span>粉丝量</span><strong>${platformRequirement.followerRequirement}</strong></div>
              <div class="task-platform-list-item"><span>达人类型</span><div>${simpleChips(platformRequirement.creatorTypes)}</div></div>
              <div class="task-platform-list-item"><span>受众性别</span><strong>${platformRequirement.audienceGender}</strong></div>
              <div class="task-platform-list-item"><span>受众年龄</span><strong>${platformRequirement.audienceAge}</strong></div>
              <div class="task-platform-list-item"><span>语言/地区要求</span><strong>${platformRequirement.language}</strong></div>
            </div>
          </section>
        </div>
      </div>
      ...
      <div class="panel task-section-card">
        ...
        <div class="task-content-block">
          <p>产品链接：${detailProfile.productLink}</p>
          <p>推广平台：${detailProfile.platforms.map((item) => item.name).join("、")}</p>
          <p>目标国家：${countryTextList(detailProfile.targetCountries)}</p>
          <p>截止报名时间：${detailProfile.deadline}</p>
          <p>达人基础要求：${detailProfile.followerRequirement}</p>
          <p>要求说明：${taskDetailProfile.contentSpec}</p>
        </div>
      </div>
      ...
      <div class="panel task-section-card">
        ...
        <div class="task-price-grid">
          <div class="task-price-card">
            <span>当前平台报价</span>
            <strong>${platformRequirement.quote}</strong>
            <small>${activePlatform.name} 平台标准</small>
          </div>
          <div class="task-price-card">
            <span>招募预算</span>
            <strong>${detailProfile.budget}</strong>
            <small>广告主当前展示预算</small>
          </div>
          <div class="task-price-card">
            <span>合作类型</span>
            <strong>${platformRequirement.contentFormat}</strong>
            <small>按当前平台规格交付</small>
          </div>
          <div class="task-price-card">
            <span>结算周期</span>
            <strong>${platformRequirement.settlementPeriod}</strong>
            <small>以当前平台结算规则为准</small>
          </div>
        </div>
      </div>
    </div>
...
function renderInvitations() {
  ...
  const pendingCount = invitations.filter((row) => row.status === "待接单").length;
  const acceptedCount = invitations.filter((row) => row.status === "已合作").length;
  return `
    ${pageHeader(
      "邀请管理",
      "合作邀请",
    ...
    <div class="tabs">
      ${tabButton("invitations", "all", `全部（${invitations.length}）`)}
      ${tabButton("invitations", "pending", `待确认接单（${pendingCount}）`)}
      ${tabButton("invitations", "accepted", `已合作（${acceptedCount}）`)}
    </div>
    <div class="table-card">
      <div class="table-head"><h2>任务名称</h2><button class="ghost-button" data-action="toast" data-message="邀请状态已刷新">刷新</button></div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>任务名称</th><th>邀请时间</th><th>合作类型</th><th>结算方式</th><th>报价(USD)</th><th>接单截止时间</th><th>状态</th><th>操作</th></tr></thead>
...
function renderMyProjects() {
  const activeCount = projectRows.filter((row) => row.status !== "已完成").length;
  const doneCount = projectRows.filter((row) => row.status === "已完成").length;
  return `
    ${pageHeader(
      "任务管理",
      "我的任务",
    ...
    <div class="tabs">
      ${tabButton("myProjects", "active", `合作中（${activeCount}）`)}
      ${tabButton("myProjects", "done", `已完成（${doneCount}）`)}
    </div>
    <div class="table-card">
...
function filteredSettingsAccountRows() {
  return settingsAccountRows
    .filter((row) => !state.adminFilters.settingsAccountNickname || row.nickname.includes(state.adminFilters.settingsAccountNickname))
    .filter((row) => !state.adminFilters.settingsAccountEmail || row.email.includes(state.adminFilters.settingsAccountEmail))
    .filter((row) => state.adminFilters.settingsAccountStatus === "全部" || row.status === state.adminFilters.settingsAccountStatus)
    .filter((row) => state.adminFilters.settingsAccountRole === "全部" || row.role === state.adminFilters.settingsAccountRole);
}
...
function renderAdminReviewQueue(pageKey, title, reviewType, pendingRows) {
  ...
  const reviewResultCounts = {
    全部: filteredRows.length,
    待审核: filteredRows.filter((row) => row.result === "待审核").length,
    已通过: filteredRows.filter((row) => row.result === "已通过").length,
    已拒绝: filteredRows.filter((row) => row.result === "已拒绝").length,
  };
  const rows = filteredRows.filter((row) => state.adminFilters.reviewResult === "全部" || row.result === state.adminFilters.reviewResult);
  return `
    ${pageHeader(
      "运营后台",
      title,
    ...
    <div class="filter-bar sticky">
      ...
      <div class="chips">
        ${["全部", "待审核", "已通过", "已拒绝"]
          .map(
            (item) => `<button class="chip ${state.adminFilters.reviewResult === item ? "active" : ""}" data-action="set-admin-review-result" data-value="${item}">${item}<span class="chip-count">${reviewResultCounts[item] || 0}</span></button>`,
          )
          .join("")}
      </div>
    </div>
...
function openAdminApplicationRejectModal(taskName = "", creatorName = "") {
  openModal(
    `
    <div class="modal-head">
      <div><h2>驳回达人报名</h2><p class="subcopy">${creatorName} · ${taskName}</p></div>
      <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
    </div>
    <div class="modal-body">
      <div class="note-box" style="margin-bottom:14px">请填写驳回原因，达人会在站内信中收到该原因。</div>
      <textarea class="textarea" id="adminApplicationRejectReason" placeholder="请输入驳回原因，例如：账号垂类不匹配、报价不符合要求、近期履约表现不达标等。"></textarea>
    </div>
    <div class="modal-foot">
      <button class="ghost-button" data-action="close-overlay">取消</button>
...
function renderEmailInvitesAdmin() {
  ...
  const historyRows = [
    {
      sentAt: "2026-6-5 10:59:20",
      subject: "官方主题邮件：邀请注册 MeetInfluencer",
      content: "Hi {{creator_name}}，我们正在邀请优质创作者加入 MeetInfluencer 平台。完成注册后，你可以收到品牌合作邀约并统一管理合作收益。",
      target: "平台潜在注册达人",
      sent: "99",
      delivered: "99 100%",
      opened: "99 100%",
    ...
    {
      sentAt: "2026-6-4 10:59:20",
      subject: "官方主题邮件：TikTok 游戏达人招募",
      content: "本周上线 TikTok 游戏专项招募，重点招募开箱、剧情、游戏体验向达人，欢迎查看合作政策并回复报名意向。",
      target: "TikTok 游戏达人名单",
      sent: "60",
      delivered: "60 100%",
      opened: "52 86.7%",
      clicked: "31 51.7%",
    ...
    {
      sentAt: "2026-6-3 10:59:20",
      subject: "官方主题邮件：英国香氛合作专场",
      content: "本次合作面向英国生活方式 / 美妆达人，支持短视频种草与开箱内容，请查看报价区间与交付说明。",
      target: "英国生活方式达人",
      sent: "50",
      delivered: "50 100%",
      opened: "41 82.0%",
    ...
    {
      sentAt: "2026-6-1 10:59:20",
      subject: "官方主题邮件：平台精选合作主题内容",
      content: "平台本周整理了 4 个高匹配合作主题，涵盖香氛、数码、饮品与游戏品类，可根据主题方向直接回复合作偏好。",
      target: "全量可邀约达人",
      sent: "150",
      delivered: "142 94.7%",
      opened: "98 69.0%",
      ...
      ${
        activeTab === "inviteList"
          ...
          : `
            ...
            <button class="ghost-button" data-action="toast" data-message="历史邮件列表已刷新">刷新</button>
          `
      }
    </div>
...
function openEmailHistoryRecordModal(subject = "", content = "", target = "") {
  openModal(
    `
      <div class="modal-head">
        <div><h2>历史邮件内容</h2><p class="subcopy">${target || "历史发送记录"}</p></div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="form-grid">
          ${field("邮件主题", `<div class="note-box">${subject || "-"}</div>`, false, "full")}
          ${field("邮件内容", `<div class="note-box">${content || "-"}</div>`, false, "full")}
        </div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">关闭</button>
        <button class="primary-button" data-action="modal-email-send">复用发送</button>
      </div>
    `,
    false,
  );
...
function openEmailImportModal() {
  openModal(
    `
      <div class="modal-head">
        <div><h2>导入达人名单</h2><p class="subcopy">支持上传名单文件，批量补充邮件邀约对象。</p></div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="form-grid">
          ${field("名单文件", '<button class="ghost-button" data-action="toast" data-message="已加入导入队列">+ 选择文件</button><small class="muted">支持 XLSX / CSV，建议包含达人昵称、邮箱、平台字段。</small>', true, "full")}
          ${field("导入说明", '<div class="note-box">导入后将进入达人邀约列表，运营可继续筛选、发送或补充状态。</div>', false, "full")}
        </div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">取消</button>
        <button class="primary-button" data-action="submit-email-import">确认导入</button>
      </div>
    `,
    true,
  );
...
function renderCampaignReviewAdmin() {
  const pendingRows = adminAdvertiserCampaignRows
    .filter((row) => row.status === "待审核")
    .map((row) => ({
      taskName: row.name,
      target: row.advertiser,
      reason: "等待运营确认活动信息完整性、预算配置与合规性。",
    }));
...
function renderCreatorDeliverableReviewAdmin() {
  const pendingRows = adminCampaignFulfillmentRows
    .filter((row) => row.operationReview === "待审核")
    .map((row) => ({
      taskName: row.campaign,
      target: `内容审核 · ${row.creator}`,
      reason: "达人已提交产物，等待运营完成初审。",
    }));
                    ...
                    .map(
                      (row) => `
                        <tr>
                          ...
                          <td>${status(row.status, row.statusClass)}</td>
                          <td><button class="link-button" data-action="open-payout-account" data-id="${row.id}">${row.account}</button></td>
                          <td>
                            <div class="actions">
                              ${
                                row.status === "待打款"
...
function renderSettingsAdmin() {
  return `
    ${pageHeader("运营后台", "系统设置", "查看基础配置与后台管理项。", `<button class="secondary-button" data-nav="settingsAccountAdmin">进入账号管理</button>`)}
    <div class="panel">
      <h2>系统设置总览</h2>
      <p class="subcopy">账号管理已拆分为独立子菜单，后续可继续扩展角色权限、通知模板与审计日志配置。</p>
    </div>
  `;
}

function renderSettingsAccountAdmin() {
  const rows = filteredSettingsAccountRows();
  return `
    ${pageHeader("运营后台", "账号管理", "管理后台账号的昵称、邮箱、状态与角色信息。")}
    <div class="table-card">
      <div class="table-head"><h2>账号列表</h2><span class="muted">共 ${rows.length} 个账号</span></div>
      <div class="filter-bar sticky">
        <input class="input" placeholder="账号昵称" value="${state.adminFilters.settingsAccountNickname}" data-admin-filter="settingsAccountNickname" />
        <input class="input" placeholder="账号邮箱" value="${state.adminFilters.settingsAccountEmail}" data-admin-filter="settingsAccountEmail" />
        <select class="select" data-admin-filter="settingsAccountStatus">
          <option value="全部" ${state.adminFilters.settingsAccountStatus === "全部" ? "selected" : ""}>账号状态</option>
          <option value="正常" ${state.adminFilters.settingsAccountStatus === "正常" ? "selected" : ""}>正常</option>
          <option value="停用" ${state.adminFilters.settingsAccountStatus === "停用" ? "selected" : ""}>停用</option>
        </select>
        <select class="select" data-admin-filter="settingsAccountRole">
          <option value="全部" ${state.adminFilters.settingsAccountRole === "全部" ? "selected" : ""}>账号角色</option>
          <option value="超级管理员" ${state.adminFilters.settingsAccountRole === "超级管理员" ? "selected" : ""}>超级管理员</option>
          <option value="运营" ${state.adminFilters.settingsAccountRole === "运营" ? "selected" : ""}>运营</option>
          <option value="审核员" ${state.adminFilters.settingsAccountRole === "审核员" ? "selected" : ""}>审核员</option>
          <option value="财务" ${state.adminFilters.settingsAccountRole === "财务" ? "selected" : ""}>财务</option>
        </select>
        <button class="primary-button" data-action="open-settings-account-create">新增账号</button>
      </div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>账号昵称</th><th>账号邮箱</th><th>状态</th><th>所属角色</th><th>创建人</th><th>创建时间</th><th>操作</th></tr></thead>
          <tbody>
            ${
              rows.length
                ? rows
                      ...
                      <tr>
                        ...
                        <td>
                          <div class="actions">
                            ${
                              row.status === "停用"
                                ? `<button class="link-button" data-action="open-settings-account-enable" data-id="${row.id}">启用</button>`
                                : `<button class="link-button" data-action="open-settings-account-disable" data-id="${row.id}">停用</button>`
                            }
                            <button class="link-button" data-action="open-settings-account-edit" data-id="${row.id}">修改</button>
                          </div>
                        </td>
                      </tr>
                    `,
...
const pages = {
  ...
  settingsAccountAdmin: renderSettingsAccountAdmin,
      ...
      <div class="table-card sample-table-card">
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>活动名称</th><th>推广地区</th><th>推广平台</th><th>合作达人</th><th>已发布达人数</th><th>作品数</th><th>预算</th><th>活动发起时间</th><th>状态</th>
              </tr>
            </thead>
            <tbody>
              ${campaigns
...
function inviteCampaignDisplay(taskName = "") {
  const profile = buildTaskProfile(taskName);
  const quoteSpec = taskQuoteSpec(taskName);
  return {
    taskName,
    platform: profile.platform || "-",
    cooperationType: profile.cooperationType || "-",
    quote: quoteSpec.mode === "range" ? `${formatUsdBudget(quoteSpec.min)} - ${formatUsdBudget(quoteSpec.max)}` : formatUsdBudget(quoteSpec.value),
    publishAt: profile.deadline || "-",
  };
}

function inviteTargetProfiles(names = []) {
  return names
    .map((name) => creators.find((item) => item.name === name) || myTalentRows.find((item) => item.name === name))
    .filter(Boolean);
}

function openInviteModal() {
  const source = state.inviteTalentSource || "recommend";
  const isTalentSearchMode = state.inviteModalMode === "talentSearch";
  const targetProfiles = inviteTargetProfiles(state.inviteSelectedCreators || []);
  const compatibleCampaigns = campaigns.filter((row) => !inviteCampaignDisabledReason(row.name, targetProfiles));
  const fallbackCampaign = compatibleCampaigns[0]?.name || campaigns[0]?.name || "";
  let selectedCampaign = state.inviteCampaignName || fallbackCampaign;
  if (isTalentSearchMode && inviteCampaignDisabledReason(selectedCampaign, targetProfiles) && fallbackCampaign) {
    selectedCampaign = fallbackCampaign;
    state.inviteCampaignName = fallbackCampaign;
  }
  ...
  openModal(
    ...
    <div class="modal-body invite-talent-body">
      ${
        isTalentSearchMode
          ? `
            <div class="invite-target-summary">
              <div class="section-row">
                <div>
                  <h3>已选邀约达人</h3>
      ...
      <div class="form-grid invite-base-form">
        ${field(
          "推广活动名称",
          `<select class="select" id="inviteCampaignSelect" data-invite-modal-field="campaign">${campaigns
            .map((row) => {
              const disabledReason = isTalentSearchMode ? inviteCampaignDisabledReason(row.name, targetProfiles) : "";
              return `<option value="${row.name}" ${row.name === selectedCampaign ? "selected" : ""} ${disabledReason ? "disabled" : ""} title="${disabledReason}">${row.name}${disabledReason ? "（平台不匹配）" : ""}</option>`;
            })
            .join("")}</select>${
            isTalentSearchMode
              ? `<small class="muted">若任务置灰，表示当前活动仅限对应平台推广，与已选达人平台不一致。</small>`
              : ""
          }`,
          true,
        )}
        ${
          isTalentSearchMode
            ? field("推广平台", `<div class="note-box">${platformDisplay}</div>`, true)
            : field("合作类型", `<select class="select" id="inviteTypeSelect"><option>短视频</option><option>直播</option><option>曝光</option><option>图文</option></select>`)
        }
        ${
          isTalentSearchMode
            ? field("合作类型", `<div class="note-box">${campaignDisplay.cooperationType}</div>`, true)
            : field("报价(USD)", `<input class="input" id="inviteQuoteInput" value="99" />`, true)
        }
        ${
          isTalentSearchMode
            ? field("报价(USD)", `<div class="note-box">${campaignDisplay.quote}</div>`, true)
            : field("期望发布时间", `<input class="input" id="invitePublishAtInput" value="2026-06-20 19:00" />`)
        }
        ${
          isTalentSearchMode
            ? field("期望发布时间", `<div class="note-box">${campaignDisplay.publishAt}</div>`, true)
            : ""
        }
        ${field("邀请备注", `<textarea class="textarea" id="inviteNoteInput">欢迎参与新品推广，期待你的创意内容。</textarea>`, false, "full")}
      </div>
      ${
        isTalentSearchMode
          ? ""
    ...
    <div class="modal-foot modal-foot-split">
      <div class="modal-foot-left"><span class="muted">${isTalentSearchMode ? `将向已选 ${targetProfiles.length} 位达人发送合作邀请` : "将向选中的达人发送合作邀请"}</span></div>
      <div class="modal-foot-right">
        <button class="ghost-button" data-action="close-overlay">取消</button>
...
function openReviewModal() {
  const creatorKey = state.currentReviewCreatorKey || "";
  openModal(
    `
    <div class="modal-head">
    ...
    <div class="modal-body">
      ${field(
        ...
        `<div class="date-range">
          <input class="input" type="date" id="brandRevisionStartDate" aria-label="开始时间" />
          <span class="date-range-sep">-</span>
          <input class="input" type="date" id="brandRevisionEndDate" aria-label="结束时间" />
        </div>`,
        true,
        "full",
      )}
      ${field("修改意见", `<textarea class="textarea" id="brandRevisionReason" placeholder="请输入修改意见">请加强产品露出，并在视频前 10 秒补充核心卖点。</textarea>`, true, "full")}
    </div>
    <div class="modal-foot">
      <button class="ghost-button" data-action="close-overlay">取消</button>
      <button class="primary-button" data-action="submit-brand-delivery-revision" data-creator="${encodeURIComponent(creatorKey)}">确认</button>
    </div>
  `,
    true,
  );
...
function openUploadModal() {
  const taskName = state.creatorUploadTask || state.creatorProjectDetailTask || findProjectRecord(state.creatorProjectDetailTask)?.task || projectRows[0]?.task || "";
  const project = findProjectRecord(taskName) || projectRows[0];
  const isPublishing = project?.progress === "待发布";
  openModal(
    `
    <div class="modal-head">
      <div><h2>${isPublishing ? "提交发布链接" : "提交产物"}</h2><p class="subcopy">${isPublishing ? "内容已通过审核，请回传最终发布链接，等待运营确认发布。" : "请根据广告主要求的内容上传脚本/视频提交。"} </p></div>
      <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
    </div>
    <div class="modal-body">
      <div class="form-grid">
        ${field("内容说明", `<textarea class="textarea" id="projectUploadSummary" placeholder="请填写本次提交内容说明，例如：脚本版本、视频时长、修改点说明等。">${isPublishing ? "已按排期完成发布，现提交正式链接供运营确认。" : ""}</textarea>`, true, "full")}
        ${field(isPublishing ? "发布链接" : "内容链接", `<input class="input" id="projectUploadLink" placeholder="请输入链接地址，支持 http 和 https" />`, isPublishing, "full")}
        ${field("附件", `<button class="ghost-button" data-action="toast" data-message="附件已加入上传队列">+ 上传文件</button>`, false, "full")}
      </div>
    </div>
    <div class="modal-foot">
      <button class="ghost-button" data-action="close-overlay">取消</button>
      <button class="primary-button" data-action="${isPublishing ? "submit-project-publish-link" : "submit-project-deliverable"}" data-task="${encodeURIComponent(taskName)}">提交</button>
    </div>
  `,
    false,
  );
...
function openProjectProgressDrawer(taskName = "任务") {
  ...
  modalRoot.innerHTML = `
    ...
    <aside class="drawer" role="dialog" aria-modal="true">
      ...
      <div class="drawer-foot">
        <button class="ghost-button" data-action="close-overlay">关闭</button>
        ${project.status === "已完成" ? "" : `<button class="primary-button" data-action="modal-upload" data-task="${encodeURIComponent(project.task)}">${project.progress === "待发布" ? "提交链接" : "提交产物"}</button>`}
      </div>
    </aside>
  `;
}
...
function payoutAccountProfile(row = {}) {
  const accountText = String(row.account || "");
  const isPaypal = accountText.toUpperCase().includes("PAYPAL");
  const emailMatch = accountText.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i);
  return {
    method: isPaypal ? "PayPal" : "Bank Transfer",
    holder: row.creator || "当前达人",
    currency: String(row.amount || "").includes("USD") ? "USD" : "USD",
    country: row.creator === "Mika Studio" ? "英国" : "未配置",
    account: emailMatch?.[0] || accountText || "未配置",
    status: "默认收款账户",
  };
}

function openPayoutAccountModal(id = "payout-1") {
  const row = payoutRows.find((item) => item.id === id) || payoutRows[0];
  const account = payoutAccountProfile(row);
  openModal(
    `
      <div class="modal-head">
        <div>
          <h2>收款账户信息</h2>
          <p class="subcopy">${row.creator} · ${row.campaign}</p>
        </div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="form-grid">
          ${field("收款方式", `<div class="note-box">${account.method}</div>`, false)}
          ${field("账户状态", `<div class="note-box">${account.status}</div>`, false)}
          ${field("持有人", `<div class="note-box">${account.holder}</div>`, false)}
          ${field("币种", `<div class="note-box">${account.currency}</div>`, false)}
          ${field("国家/地区", `<div class="note-box">${account.country}</div>`, false)}
          ${field("账户信息", `<div class="note-box">${account.account}</div>`, false, "full")}
        </div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">关闭</button>
...
function findSettingsAccountRow(id = "") {
  return settingsAccountRows.find((row) => row.id === id) || settingsAccountRows[0];
}

function settingsAccountRoleOptions(selected = "") {
  return ["超级管理员", "运营", "审核员", "财务"]
    .map((item) => `<option value="${item}" ${selected === item ? "selected" : ""}>${item}</option>`)
    .join("");
}

function openSettingsAccountCreateModal() {
  openModal(
    `
      <div class="modal-head">
        <div><h2>新增账号</h2><p class="subcopy">创建新的后台管理账号。</p></div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="form-grid">
          ${field("账号昵称", '<input class="input" id="settingsAccountNicknameInput" placeholder="请输入账号昵称" />', true)}
          ${field("账号邮箱", '<input class="input" id="settingsAccountEmailInput" placeholder="请输入账号邮箱" />', true)}
          ${field("所属角色", `<select class="select" id="settingsAccountRoleInput">${settingsAccountRoleOptions("运营")}</select>`, true)}
          ${field("状态", '<div class="note-box">默认创建为正常</div>', false)}
        </div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">取消</button>
        <button class="primary-button" data-action="submit-settings-account-create">确认新增</button>
      </div>
    `,
    true,
  );
...
function openSettingsAccountEditModal(id = "") {
  const row = findSettingsAccountRow(id);
  openModal(
    `
      <div class="modal-head">
        <div><h2>修改账号</h2><p class="subcopy">${row.email}</p></div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="form-grid">
          ${field("账号昵称", `<input class="input" id="settingsAccountEditNicknameInput" value="${row.nickname}" placeholder="请输入账号昵称" />`, true)}
          ${field("账号邮箱", `<div class="note-box">${row.email}</div>`, false)}
          ${field("所属角色", `<select class="select" id="settingsAccountEditRoleInput">${settingsAccountRoleOptions(row.role)}</select>`, true)}
          ${field("当前状态", `<div class="note-box">${row.status}</div>`, false)}
        </div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">取消</button>
        <button class="primary-button" data-action="submit-settings-account-edit" data-id="${row.id}">保存修改</button>
      </div>
    `,
    true,
  );
...
function openSettingsAccountDisableModal(id = "") {
  const row = findSettingsAccountRow(id);
  openModal(
    `
      <div class="modal-head">
        <div><h2>确认停用账号</h2><p class="subcopy">${row.nickname} · ${row.email}</p></div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="note-box">停用后，该账号将无法继续登录后台或执行当前角色对应操作。请确认是否继续停用。</div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">取消</button>
        <button class="danger-button" data-action="submit-settings-account-disable" data-id="${row.id}">确认停用</button>
      </div>
    `,
    true,
  );
...
function openSettingsAccountEnableModal(id = "") {
  const row = findSettingsAccountRow(id);
  openModal(
    `
      <div class="modal-head">
        <div><h2>确认启用账号</h2><p class="subcopy">${row.nickname} · ${row.email}</p></div>
        <button class="close-button" data-action="close-overlay" aria-label="关闭">×</button>
      </div>
      <div class="modal-body">
        <div class="note-box">启用后，该账号将恢复后台登录和角色权限，请确认是否继续启用。</div>
      </div>
      <div class="modal-foot">
        <button class="ghost-button" data-action="close-overlay">取消</button>
        <button class="primary-button" data-action="submit-settings-account-enable" data-id="${row.id}">确认启用</button>
      </div>
    `,
    true,
  );
...
function renderSubmissionHistoryPanel(history = []) {
  if (!history.length) return "";
  return `
    <div class="fulfillment-submission-list" aria-label="产物提交记录">
      <div class="fulfillment-submission-title">本轮产物提交</div>
      ${history
        .map(
          (item) => `
            <div class="fulfillment-submission-card">
              <div class="fulfillment-submission-head">
                <strong>第 ${item.round || 1} 次提交</strong>
                <span>${item.submittedAt || "-"}</span>
              </div>
              <p>${item.version || "-"} · ${item.summary || "-"}</p>
              <div class="fulfillment-submission-review">运营审核：${item.reviewResult || "待运营审核"}${item.reviewReason ? `；说明：${item.reviewReason}` : ""}</div>
              ${
                item.brandReviewResult && item.brandReviewResult !== "-"
                  ? `<div class="fulfillment-submission-review">广告主审核：${item.brandReviewResult}${item.brandReviewReason ? `；说明：${item.brandReviewReason}` : ""}</div>`
                  : ""
              }
            </div>
          `,
...
function openDeliveryDrawer(creatorKey = "") {
  ...
  const submissionHistoryPanel = renderSubmissionHistoryPanel(submissionHistory);
  const timelineItems = buildFulfillmentFlow(row).map((item) => {
    const nextItem = { ...item };
    if (submissionHistoryPanel && item.title === "内容初审中（平台）") nextItem.submissionPanel = submissionHistoryPanel;
    if (isAdvertiserReviewStage && item.title === "内容复审中（广告主）") nextItem.deliverablePanel = deliverableMarkup;
    return nextItem;
  });
  const footer =
    isAdvertiserReviewStage
      ? `
        <div class="drawer-foot">
          <button class="secondary-button" data-action="modal-review" data-creator="${encodeURIComponent(row.creatorId || row.creator)}">要求修改</button>
  ...
  modalRoot.innerHTML = `
    ...
    <aside class="drawer" role="dialog" aria-modal="true">
      ...
      <div class="drawer-foot">
        <button class="ghost-button" data-action="close-overlay">关闭</button>
        ${
          row.publishStatus === "待确认"
            ? `<button class="primary-button" data-action="confirm-admin-publish" data-task="${encodeURIComponent(row.campaign)}" data-creator="${encodeURIComponent(row.creator)}">确认发布</button>`
            : ""
        }
      </div>
    </aside>
...
function handleAction(action, target) {
  ...
  if (action === "resubmit-campaign") {
    const taskName = decodeURIComponent(target.dataset.task || "");
    preloadPublishDraftForCampaign(taskName);
    state.publishStep = 0;
    state.publishDraft.matchDrawerOpen = false;
    openPublishModal();
    toast("已载入被驳回活动，请补充信息后重新提交");
    return;
  }
  ...
  if (action === "copy-previous-platform-requirements") {
    ...
    state.publishDraft.platformConfigs[targetPlatform] = {
      ...
      cooperationValue: previous.cooperationValue,
  ...
  if (action === "approve-admin-campaign-review") {
    const taskName = decodeURIComponent(target.dataset.task || "");
    const reviewTarget = decodeURIComponent(target.dataset.target || "当前广告主");
    ...
    if (campaign) {
      campaign.status = "进行中";
      campaign.statusClass = "running";
    }
    const adminRow = findAdminCampaignRecord(taskName);
    if (adminRow) {
      adminRow.status = "招募中";
      adminRow.statusClass = "pending";
    }
    ensureCreatorCampaignVisible(taskName);
    notifyBrand({
      id: `brand-campaign-approved-${Date.now()}`,
      type: "campaign_approved",
      task: taskName,
      title: "活动审核已通过",
      content: `你创建的「${taskName}」已通过运营审核，达人现在可以在机会大厅中看到该活动。`,
      cta: "查看活动",
    });
    render();
    toast("已通过当前活动审核");
  ...
  if (action === "submit-admin-campaign-reject") {
    ...
    if (campaign) {
      campaign.status = "已驳回";
      campaign.statusClass = "rejected";
    }
    const adminRow = findAdminCampaignRecord(taskName);
    if (adminRow) {
      adminRow.status = "已拒绝";
      adminRow.statusClass = "rejected";
    }
    notifyBrand({
      id: `brand-campaign-rejected-${Date.now()}`,
      type: "campaign_rejected",
      task: taskName,
      title: "活动审核未通过",
      content: `你创建的「${taskName}」未通过运营审核，原因：${reason}`,
      cta: "查看原因",
    });
    closeOverlay();
    render();
  ...
  if (action === "approve-admin-application-review") {
    const taskName = decodeURIComponent(target.dataset.task || "");
    const creatorName = decodeURIComponent(target.dataset.creator || "");
    const record = findApplicationRecord(taskName, creatorName) || applicationRecords.find((item) => item.task === taskName);
    if (record) {
      record.statusKey = "approved";
      record.feedbackTime = formatNow();
      record.note = "运营审核通过，已进入合作流程。";
    }
    adminReviewHistoryRows.unshift({
      taskName,
      target: `达人报名审核 · ${creatorName}`,
      reviewType: "达人报名",
      result: "已通过",
      reviewer: "运营 · 当前账号",
      time: formatNow(),
    ...
    notifyCreator({
      id: `creator-apply-approved-${Date.now()}`,
      type: "apply_approved",
      task: taskName,
      title: "报名结果",
      content: `你报名的「${taskName}」已通过审核，已进入合作流程。`,
      cta: "查看报名",
      statusKey: "approved",
    });
    closeOverlay();
  ...
  if (action === "submit-admin-application-reject") {
    const reason = document.getElementById("adminApplicationRejectReason")?.value.trim() || "";
    if (!reason) {
      toast("请先填写驳回理由");
      return;
    }
    ...
    notifyCreator({
      id: `creator-apply-rejected-${Date.now()}`,
      type: "apply_rejected",
      task: taskName,
      title: "报名结果",
      content: `你报名的「${taskName}」未通过审核，原因：${reason}`,
      cta: "查看原因",
      statusKey: "rejected",
    });
    closeOverlay();
  ...
  if (action === "approve-admin-deliverable-review") {
    ...
    appendProjectTimeline(taskName, "运营初审通过", "运营已完成内容初审，已流转给广告主复审。", "待广告主审核");
    const collab = findCollaborationRecord(taskName, creatorName);
    if (collab) {
      collab.status = "待审核";
      collab.statusClass = "pending";
      collab.progress = "待广告主审核";
      collab.note = "运营初审已通过，等待广告主复审。";
    }
    const fulfillment = findFulfillmentRecord(taskName, creatorName);
    if (fulfillment) {
  ...
  if (action === "submit-admin-deliverable-reject") {
    ...
    appendProjectTimeline(taskName, "运营初审驳回", reason, "审核被拒");
    const collab = findCollaborationRecord(taskName, creatorName);
    if (collab) {
      collab.status = "进行中";
      collab.statusClass = "rejected";
      collab.progress = "审核被拒";
      collab.rejectReason = reason;
      collab.note = "运营已驳回当前产物，等待达人修改后重新提交。";
    }
    const fulfillment = findFulfillmentRecord(taskName, creatorName);
    if (fulfillment) {
      fulfillment.deliverableStatus = "待达人修改";
      fulfillment.operationReview = "已拒绝";
    ...
    notifyCreator({
      ...
      task: taskName,
      title: "产物审核结果",
      content: `你提交的「${taskName}」未通过运营初审，原因：${reason}`,
      cta: "查看原因",
    });
    closeOverlay();
    render();
  ...
  if (action === "open-email-history-record") {
    openEmailHistoryRecordModal(
      decodeURIComponent(target.dataset.subject || ""),
      decodeURIComponent(target.dataset.content || ""),
      decodeURIComponent(target.dataset.target || ""),
    );
    return;
  }
  if (action === "submit-email-import") {
    closeOverlay();
    toast("达人名单已导入");
    return;
  }
  ...
  if (action === "submit-publish") {
    const taskName = ensureCampaignLifecycleRowFromDraft();
    state.publishStep = 0;
    state.publishDraft.matchDrawerOpen = false;
    state.brandHasCampaigns = true;
    closeOverlay();
    notifyBrand({
      id: `brand-campaign-submit-${Date.now()}`,
  ...
  if (action === "toggle-talent-invite-selection") {
    const name = decodeURIComponent(target.dataset.name || "");
    const next = new Set(state.talentInviteSelections || []);
    if (target.checked) next.add(name);
    else next.delete(name);
    state.talentInviteSelections = [...next];
    render();
    return;
  }
  if (action === "toggle-talent-invite-select-all") {
    const visibleNames = filterCreators(creators).map((row) => row.name);
    const next = new Set(state.talentInviteSelections || []);
    if (target.checked) visibleNames.forEach((name) => next.add(name));
    else visibleNames.forEach((name) => next.delete(name));
    state.talentInviteSelections = [...next];
    render();
    return;
  }
  if (action === "open-talent-search-invite") {
    const creatorName = decodeURIComponent(target.dataset.creator || "");
    const selectedCreators = creatorName ? [creatorName] : [...new Set(state.talentInviteSelections || [])];
    if (!selectedCreators.length) {
      toast("请先选择至少 1 位达人");
      return;
    }
    state.inviteModalMode = "talentSearch";
    state.inviteSelectedCreators = selectedCreators;
    state.inviteCampaignName = state.inviteCampaignName || campaigns[0]?.name || "";
    openInviteModal();
    return;
  }
  ...
  if (action === "modal-invite") {
    state.inviteModalMode = "default";
    state.inviteCampaignName = state.inviteCampaignName || campaigns[0]?.name || "";
    openInviteModal();
  }
  ...
  if (action === "open-payout-account") {
    openPayoutAccountModal(target.dataset.id || "payout-1");
    return;
  }
  ...
  if (action === "open-settings-account-create") {
    openSettingsAccountCreateModal();
    return;
  }
  if (action === "open-settings-account-edit") {
    openSettingsAccountEditModal(target.dataset.id || "");
    return;
  }
  if (action === "open-settings-account-disable") {
    openSettingsAccountDisableModal(target.dataset.id || "");
    return;
  }
  if (action === "open-settings-account-enable") {
    openSettingsAccountEnableModal(target.dataset.id || "");
    return;
  }
  ...
  if (action === "submit-payout-paid") {
    ...
    if (row) {
      ...
      row.note = "已完成人工打款登记。";
      row.rejectReason = "";
      const fulfillment = findFulfillmentRecord(row.campaign, row.creator);
      if (fulfillment) fulfillment.settlementStatus = "已结算";
    }
    closeOverlay();
    render();
    toast("已标记为已打款");
  ...
  if (action === "submit-payout-reject") {
    ...
    if (row) {
      ...
      row.note = "该笔打款已被人工驳回。";
      const fulfillment = findFulfillmentRecord(row.campaign, row.creator);
      if (fulfillment) fulfillment.settlementStatus = "打款驳回";
    }
    closeOverlay();
    render();
    toast("已驳回该笔结算");
  ...
  if (action === "submit-settings-account-create") {
    const nickname = document.getElementById("settingsAccountNicknameInput")?.value.trim() || "";
    const email = document.getElementById("settingsAccountEmailInput")?.value.trim() || "";
    const role = document.getElementById("settingsAccountRoleInput")?.value || "运营";
    if (!nickname || !email) {
      toast("请补全账号昵称和账号邮箱");
      return;
    }
    settingsAccountRows.unshift({
  ...
  if (action === "submit-settings-account-edit") {
    const row = findSettingsAccountRow(target.dataset.id || "");
    const nickname = document.getElementById("settingsAccountEditNicknameInput")?.value.trim() || "";
    const role = document.getElementById("settingsAccountEditRoleInput")?.value || "运营";
    if (!nickname) {
      toast("请填写账号昵称");
      return;
    }
  ...
  if (action === "submit-settings-account-disable") {
    const row = findSettingsAccountRow(target.dataset.id || "");
    row.status = "停用";
    row.statusClass = "rejected";
    closeOverlay();
  ...
  if (action === "submit-settings-account-enable") {
    const row = findSettingsAccountRow(target.dataset.id || "");
    row.status = "正常";
  ...
  if (action === "confirm-invite") {
    ...
    const taskName = document.getElementById("inviteCampaignSelect")?.value || state.inviteCampaignName || campaigns[0]?.name || "";
    const selectedProfiles = inviteTargetProfiles(state.inviteSelectedCreators || []);
    const disabledReason = inviteCampaignDisabledReason(taskName, selectedProfiles);
    if (disabledReason) {
      toast(disabledReason, "无法发送邀请");
      return;
    }
    const linkedCampaign = inviteCampaignDisplay(taskName);
    const type = document.getElementById("inviteTypeSelect")?.value || linkedCampaign.cooperationType || "短视频";
    const quote = document.getElementById("inviteQuoteInput")?.value || linkedCampaign.quote || "99";
    const publish = document.getElementById("invitePublishAtInput")?.value || linkedCampaign.publishAt || "-";
    const advertiser = findAdminCampaignRecord(taskName)?.advertiser || "当前广告主";
    state.inviteSelectedCreators.forEach((creatorName) => {
      const existing = invitations.find((row) => row.task === taskName && row.creator === creatorName);
      const row = {
        task: taskName,
        advertiser,
        creator: creatorName,
  ...
  if (action === "modal-review") {
    state.currentReviewCreatorKey = decodeURIComponent(target.dataset.creator || state.currentReviewCreatorKey || "");
    openReviewModal();
  }
  ...
  if (action === "set-task-detail-platform") {
    state.taskDetailPlatform = target.dataset.platform || "";
    render();
    return;
  }
  if (action === "modal-upload") {
    state.creatorUploadTask = decodeURIComponent(target.dataset.task || state.creatorProjectDetailTask || "");
    openUploadModal();
  }
  ...
  if (action === "submit-project-deliverable") {
    const taskName = decodeURIComponent(target.dataset.task || state.creatorUploadTask || "");
    const summary = document.getElementById("projectUploadSummary")?.value.trim() || "达人已提交本轮产物，等待运营审核。";
    const link = document.getElementById("projectUploadLink")?.value.trim() || "";
    const project = findProjectRecord(taskName);
    ...
    const fulfillment = findFulfillmentRecord(taskName, creatorName);
    if (fulfillment) {
      fulfillment.cooperationStatus = "履约中";
      fulfillment.deliverableStatus = "内容审核中";
      fulfillment.operationReview = "待审核";
      fulfillment.brandReview = "-";
  ...
  if (action === "submit-project-publish-link") {
    const taskName = decodeURIComponent(target.dataset.task || state.creatorUploadTask || "");
    const summary = document.getElementById("projectUploadSummary")?.value.trim() || "达人已完成发布并提交正式链接。";
    const link = document.getElementById("projectUploadLink")?.value.trim() || "";
    if (!link) {
      toast("请先填写发布链接");
      return;
    ...
    const project = findProjectRecord(taskName);
    const fulfillment = findFulfillmentRecord(taskName);
    ...
    appendProjectTimeline(taskName, "达人提交发布链接", summary, "待确认发布");
    if (collab) {
      collab.progress = "待确认发布";
      collab.status = "进行中";
      collab.statusClass = "pending";
      collab.publishTime = formatNow();
      collab.note = "达人已提交正式发布链接，等待运营确认发布。";
      collab.tracking = link;
    }
    if (fulfillment) {
      fulfillment.deliverableStatus = "待确认发布";
      fulfillment.publishStatus = "待确认";
      fulfillment.trackingStatus = "待确认";
    }
    appendFulfillmentTimeline(taskName, fulfillment?.creatorId || collab?.creatorId, "提交发布链接", "达人已提交正式发布链接，等待运营确认发布。");
    closeOverlay();
    render();
    toast("发布链接已提交，等待运营确认");
    return;
  }
  if (action === "submit-brand-delivery-revision") {
    const creatorKey = decodeURIComponent(target.dataset.creator || state.currentReviewCreatorKey || "");
    const reason = document.getElementById("brandRevisionReason")?.value.trim() || "";
    if (!reason) {
      toast("请先填写修改意见");
      return;
    }
    ...
    if (row) {
      ...
      row.statusClass = "rejected";
      row.rejectReason = reason;
      row.note = "广告主已驳回当前版本，等待达人修改后重新提交。";
      ...
      appendProjectTimeline(row.campaign, "广告主审核驳回", reason, "审核被拒");
      const fulfillment = findFulfillmentRecord(row.campaign, row.creator);
      if (fulfillment) {
        fulfillment.deliverableStatus = "待达人修改";
      ...
      notifyCreator({
        ...
        task: row.campaign,
        title: "产物审核结果",
        content: `你提交的「${row.campaign}」未通过广告主复审，原因：${reason}`,
        cta: "查看原因",
      });
    }
    closeOverlay();
  ...
  if (action === "approve-collab-delivery") {
    ...
    if (row) {
      ...
      appendProjectTimeline(row.campaign, "广告主审核通过", "广告主已确认内容终稿，等待达人完成正式发布。", "待发布");
      const fulfillment = findFulfillmentRecord(row.campaign, row.creator);
      if (fulfillment) {
        fulfillment.deliverableStatus = "待发布";
        fulfillment.brandReview = "已通过";
      ...
      notifyCreator({
        ...
        task: row.campaign,
        title: "产物审核结果",
        content: `你提交的「${row.campaign}」内容已通过审核，请按排期完成发布并提交链接。`,
        cta: "查看结果",
      });
    }
    closeOverlay();
  ...
  if (action === "confirm-admin-publish") {
    const taskName = decodeURIComponent(target.dataset.task || "");
    const creatorName = decodeURIComponent(target.dataset.creator || "");
    const project = findProjectRecord(taskName);
    const fulfillment = findFulfillmentRecord(taskName, creatorName);
    const collab = findCollaborationRecord(taskName, creatorName);
    if (project) {
      project.progress = "已完成发布";
      project.status = "已完成";
      project.statusClass = "done";
      project.finalStatus = "已完成发布";
    }
    appendProjectTimeline(taskName, "运营确认发布", "运营已确认正式发布，系统将自动生成结算单。", "进入结算");
    if (collab) {
      collab.status = "已发布";
      collab.statusClass = "published";
      collab.progress = "效果追踪";
      collab.publishTime = formatNow();
      collab.note = "运营已确认发布，等待效果回流与结算。";
      collab.tracking = collab.tracking || "已回传发布链接";
      collab.settleAmount = `$${String(project?.quote || collab.price || "0").replace(/[^\d.]/g, "")}`;
    }
    if (fulfillment) {
      fulfillment.deliverableStatus = "已发布";
      fulfillment.publishStatus = "已发布";
      fulfillment.trackingStatus = "追踪中";
      fulfillment.settlementStatus = "待结算";
    }
    const campaign = findCampaignRecord(taskName);
    if (campaign) {
      campaign.publishedCreators = Number(campaign.publishedCreators || 0) + 1;
      campaign.works = Number(campaign.works || 0) + 1;
    }
    const adminRow = findAdminCampaignRecord(taskName);
    if (adminRow) adminRow.published = Number(adminRow.published || 0) + 1;
    appendFulfillmentTimeline(taskName, fulfillment?.creatorId || collab?.creatorId, "确认发布", "运营已确认内容正式发布，系统生成结算单。");
    ensureSettlementRecord(taskName, creatorName || collab?.creator || "Mika Studio", project?.quote || collab?.price || "0");
    closeOverlay();
    render();
    toast("已确认发布，系统已生成结算单");
    return;
  }
...
document.addEventListener("input", (event) => {
  ...
  if (key === "cooperationTypeSingle") {
    ...
    if (!fieldTarget.value) state.publishDraft.platformConfigs[platform].cooperationValue = "";
  } else {
...
document.addEventListener("change", (event) => {
  const inviteTarget = event.target.closest("[data-invite-modal-field='campaign']");
  if (!inviteTarget) return;
  state.inviteCampaignName = String(inviteTarget.value || campaigns[0]?.name || "");
  openInviteModal();
});
